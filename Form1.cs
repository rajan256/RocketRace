...
namespace RocketRace
{
    public partial class Form1 : Form
    {
        ...
        private void WinningDetails(int WinningSpace)
        {
            for (int i = 0; i < 3; i++)
            {
                if (WinningSpace == myguy[i].Spaceship) //comparing the winning ship with the ship that the punter chosen
                {
                    ...
                    {
                        case 0:
                            txtJonny.Text = string.Format("{0} " + "Won and now has got ${1} ", myguy[0].Name, myguy[0].Cash);
                            break;
                ...
                else
                {
                    ...
                    {
                        case 0:
                            txtJonny.Text = string.Format("{0} " + "Lost and now has got ${1} ", myguy[0].Name, myguy[0].Cash);
                            break;
                ...
                if (myguy[i].Cash == 0) // if guys don't have money at all
                {
                    ...
                    {
                        case 0:
                            JonnyRadioButton.Enabled = false;
                            txtJonny.ForeColor = Color.Red;
                            txtJonny.Text = "BUSTED";
                            break;
                        case 1:
                ...
                if (JonnyRadioButton.Enabled == false && AnilRadioButton.Enabled == false && // Each of Radiobuttons are inactiv
                   AlRadioButton.Enabled == false)
                {
        ...
        private void btnPlaceBet_Click_1(object sender, EventArgs e)
        {
            ...
            if (myguy[Factory.GuyNumber].Name == "Jonny")
            {
                //we can see the name, amount and the spaceship number on Jonny's textbox 
                txtJonny.Text = string.Format("{0} bets $ {1} on Spaceship Number {2}",
